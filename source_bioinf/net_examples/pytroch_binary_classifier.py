...
PATH_TO_FASTA_FILE_POS = '/mnt/data1/evo_brain_results/selected_data_intersection/chbrain/Chimpanzee_Cerebellum.fasta'
PATH_TO_FASTA_FILE_NEG = '/mnt/data1/evo_brain_results/selected_data_intersection/chbrain_rnd_neg_liftover_from_human/1_to_1_ratio/Human_Cerebellum.rnd_sampled_liftover.fasta'
PATH_TO_VALIDATION_FILE_POS = '/mnt/data1/evo_brain_results/selected_data_intersection/chbrain/Chimpanzee_Cerebellum.fasta'
PATH_TO_VALIDATION_FILE_NEG = '/mnt/data1/evo_brain_results/selected_data_intersection/chbrain_rnd_neg_liftover_from_human/1_to_1_ratio/Human_Cerebellum.rnd_sampled_liftover.fasta'
...
parser.add_option("-p", "--input_pos_path_file", action="store", type="string", dest="pos_path_file",
                  help='Path to input data for training positives')

parser.add_option("-n", "--input_neg_path_file", action="store", type="string", dest="neg_path_file",
                  help='Path to input data for training negatives')

parser.add_option("-i", "--input_y_path_file", action="store", type="string", dest="pos_val_path_file",
                  help='Path to answers for input data for validation')

parser.add_option("-j", "--output_scores_save_path", action="store", type="string", dest="neg_val_path_file",
                  help='Path to answers for input data for validation')

parser.add_option("-r", "--output_model_save_path", action="store", type="string", dest="save_roc_score_path",
                  help='Folder to save output models')

parser.add_option("-s", "--output_correlations_model_save_path", action="store", type="string", dest="save_raw_score_path",
                  help='Folder to save output models')

(options, args) = parser.parse_args()

PATH_TO_FASTA_FILE_POS = options.pos_path_file
PATH_TO_FASTA_FILE_NEG = options.neg_path_file
PATH_TO_VALIDATION_FILE_POS = options.pos_val_path_file
PATH_TO_VALIDATION_FILE_NEG = options.neg_val_path_file
PATH_TO_SAVE_PREDICTION_SCORES = options.save_roc_score_path
FOLDER_TO_SAVE_MODEL = "../pytorch_model_predictions/"
PATH_TO_SAVE_AUROC_AUPR_SCORES = options.save_raw_score_path
...
def trainNet(net, batch_size, n_epochs, learning_rate, dataset_train, val_loader, use_gpu=False):
    if use_gpu:
        net.to(DEVICE_ID)
...
# X_train = X_train[:300,:,:]
# X_train_val = X_train_val[:300,:,:]
# Y_train = Y_train[:300,:]
# Y_train_val = Y_train_val[:300,:]
...
trainNet(CNN, batch_size=BATCH_SIZE, n_epochs=200, learning_rate=LEARNING_RATE, dataset_train=train_data,
         val_loader=val_loader, use_gpu=use_gpu)
