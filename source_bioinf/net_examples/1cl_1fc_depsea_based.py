...
def get_scores_pos_class_binary_sigmoid(tensor_with_true_scores):
    vector_with_labels = []
    pos_idx = 1
    neg_idx = 0
    for row_idx in range(0, tensor_with_true_scores.size()[0]):
...
class Multiple_Input_Model(nn.Module):
    def __init__(self):
        ...
        self.do = nn.Dropout(p=0.10)
        self.do_conv = nn.Dropout(p=0.10)
        self.l1 = nn.Linear(74240, 1)
...
class DeepSEA(nn.Module):
    def __init__(self, sequence_length, n_genomic_features):
        ...
        conv_kernel_size = 8
        pool_kernel_size = 4
        # n_output_features_fc1 = 1000
        # n_output_features_fc1 = 1


        self.conv_net = nn.Sequential(
            nn.Conv1d(4, 1000, kernel_size=conv_kernel_size),
    ...
    def forward(self, x):
        ...
        """
        reshape_out = self.conv_net(x)
        N, _, _ = reshape_out.size()
        reshape_out = reshape_out.view(N, -1)
        # reshape_out = reshape_out.view(reshape_out.size(0), 320 * self.n_channels)
...
def get_optimizer(model_parameters):
    ...
    """
    optimizer = optim.Adam(model_parameters, weight_decay=1e-6)
    return optimizer
...
def trainNet(net, batch_size, n_epochs, learning_rate, dataset_train, val_loader, label_text_id_from_index_dict, true_val_labels, use_gpu=False):
    if use_gpu:
        net.to(DEVICE_ID)
    ...
    # loss = criterion()
    # optimizer = get_optimizer(net.parameters(), learning_rate)
...
CNN = DeepSEA(sequence_length=600, n_genomic_features = 2)
...
trainNet(CNN, batch_size=BATCH_SIZE,  n_epochs=200, learning_rate=LEARNING_RATE, dataset_train=train_data,
         val_loader=val_loader, label_text_id_from_index_dict = region_ID_2_ylabel, true_val_labels=y_labels_val, use_gpu=use_gpu)
